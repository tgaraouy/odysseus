...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Mohtasib Chat';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to Mohtasib kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'Mohtasib Chat';
    }
