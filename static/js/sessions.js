...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Helm Chat';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to Helm kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'Helm Chat';
    }
