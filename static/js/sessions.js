...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Touf Chat';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to Touf kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'Touf Chat';
    }
