    ...
    def build_context_preface(
        ...
        *anything* that changes turn-to-turn — timestamps, retrieved snippets,
        ...
        if use_memory:
            ...
            _used_ids: list = []
            if pinned:
                # Provenance guard (F-3): only USER-sourced pinned memories may wear
                # the authoritative "Core facts about the user" label. Memories saved
                # by the agent or auto-extraction (source != "user") could themselves
                # originate from untrusted content the agent ingested earlier; if
                # pinned, presenting them as core user facts would launder
                # attacker-influenced text into a trusted-looking frame. Route those
                # through a neutral label. Both groups stay fenced as untrusted; only
                # the label authority differs. Missing source defaults to "user".
