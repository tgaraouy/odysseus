...
### F-4 — Fence-escape weakness in the wrappers  *(ALREADY UPSTREAM — reused)*
**Severity: Medium.** Wrappers delimit untrusted text with literal sentinels
...
**Status:** independently fixed upstream in **#3086** — `prompt_security._escape_guard_markers()`
defangs embedded guard markers (and the label is sanitized + moved inside the guard). Our F-1
tool-output wrapper **reuses `_escape_guard_markers`** and additionally defangs its own
tool-output sentinels, so the same protection covers mid-loop output. No separate change
needed here.
...
| File | Change |
|---|---|
| `src/prompt_security.py` | New `UNTRUSTED_OUTPUT_TOOLS`, `is_untrusted_tool_output()`, `wrap_untrusted_tool_output()` — reuses upstream `_escape_guard_markers` for anti-escape (F-1; F-4 already upstream #3086). |
...
| `src/tool_execution.py` | `_is_write_protected_path()` + `for_write` flag on `_resolve_tool_path`/`_resolve_tool_path_in_workspace` (F-6); Tier-A helpers `_sandbox_env`/`_sandbox_preexec`/`sandbox_subproc_cwd` + `_format_sandbox_result` (F-7 / Tier A/B). |
| `src/agent_tools/filesystem_tools.py` | `write_file`/`edit_file` pass `for_write=True` so they can't write the ledger subtree (F-6). |
| `src/agent_tools/subprocess_tools.py` | `bash`/`python` handlers: scrubbed env + scratch cwd + rlimits (Tier A); dispatch into the sidecar runner, fail-closed (Tier B). |
| `tests/test_untrusted_tool_output.py` | classification (incl. fail-closed + mcp\_\_\*), wrapping, fence-escape. |
| `tests/test_ledger_write_protection.py` | ledger write blocked, read allowed, normal data writable, `LEDGER_DIR` override (F-6). |

56 security tests pass on the current `dev` base. Note the codebase was refactored upstream
(execution → `src/agent_tools/`); the fixes are applied at the relocated chokepoints.
