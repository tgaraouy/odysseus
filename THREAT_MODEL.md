...
1. **Shell/filesystem sandbox — ADDRESSED (this deployment).** `bash`/`python` now dispatch into an isolated sidecar (`docker/sandbox/`): `network_mode:none`, read-only rootfs, tmpfs scratch, `cap_drop:ALL`, non-root, no data/secret/.ssh mounts, fail-closed. Plus in-process Tier A (env-scrub + scratch cwd + rlimits) and opt-in Tier C (egress allowlist). See `docs/security/bash-python-sandbox-scoping.md`. (Upstream proposal #1058.)

2. **SSRF via `/api/v1/chat` `base_url` — ADDRESSED.** Token-supplied `base_url` is validated by `src/url_security.py:validate_public_http_url` before any request: non-http(s) schemes rejected; `localhost`/`metadata`/`.internal`/`.lan` etc. blocked; all private/loopback/link-local/CGNAT/reserved ranges blocked (incl. `169.254.169.254` and `host.docker.internal`); DNS resolved with all-IPs-must-be-public, fail-closed. httpx does not follow redirects (no `follow_redirects=True` anywhere). Covered by `tests/test_url_safety.py` + `tests/test_webhook_ssrf_resilience.py`. **Residual:** DNS-rebinding TOCTOU (validate-time vs request-time re-resolution) — not eliminated; would require pinning the validated IP into the connection.
...
4. **Token scopes — per-capability exists and is enforced on the API surface; the residual is agent-mediated access.** API tokens carry granular scopes (`chat`, `todos:read|write`, `documents:read|write`, `email:read|draft|send`, `calendar:read|write`, `memory:read|write` — `routes/api_token_routes.py:ALLOWED_SCOPES`), enforced on the external routes (`routes/codex_routes.py`, and `/v1/chat` requires `chat`). **Residual gap:** scopes gate *direct API endpoints*, but once a `chat`-scoped token drives the **agent**, the agent runs with the token *owner's full tool access* via the internal-tool loopback — the token's scope does not constrain the agent's tools. So a leaked companion token can be conversationally steered into shell/email/file actions if the owner is admin. Fix = intersect the agent's available tools with the authenticating token's scopes (gate `disabled_tools` by scope in the chat path) — affects API-token sessions only, not the admin's own session.
